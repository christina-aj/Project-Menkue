...
class Stock extends CI_Controller {
	...
	public function print_stock_in()
	{
		$data['row'] = $this->stock_m->get_stock_in()->result();
        $this->template->load('template', 'print/print_stock_in', $data);
	}
    ...
    public function stock_in_del()
    {
        ...
        if($this->db->affected_rows() > 0){
            $this->session->set_flashdata('success','Data Stock Masuk Berhasil Dihapus');
        }
        redirect('stock/in');
    }
	...
	public function print_stock_out()
	{
		$data['row'] = $this->stock_m->get_stock_out()->result();
        $this->template->load('template', 'print/print_stock_out', $data);
	}
